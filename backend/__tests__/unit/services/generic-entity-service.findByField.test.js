...
describe('GenericEntityService.findByField()', () => {
  ...
  describe('validation', () => {
    test('should throw error for invalid entity name', async () => {
      await expect(GenericEntityService.findByField('invalid_entity', 'email', 'test@test.com'))
    ...
    test('should throw error for non-filterable field', async () => {
      ...
      await expect(GenericEntityService.findByField('user', 'password', 'secret'))
        .rejects.toThrow("Field 'password' is not filterable for user");
    });

    test('should throw error for field not in filterableFields', async () => {
      await expect(GenericEntityService.findByField('customer', 'secret_field', 'value'))
  ...
  describe('successful queries', () => {
    test('should find user by email', async () => {
      ...
      const result = await GenericEntityService.findByField('user', 'email', 'test@example.com');
    ...
    test('should find user by auth0_id with role JOIN (defaultIncludes)', async () => {
      const mockUser = { id: 1, email: 'test@example.com', auth0_id: 'auth0|abc123', role: 'admin' };
      db.query.mockResolvedValueOnce({ rows: [mockUser] });

      const result = await GenericEntityService.findByField('user', 'auth0_id', 'auth0|abc123');
    ...
    test('should find customer by email', async () => {
      const mockCustomer = { id: 1, email: 'cust@example.com', company_name: 'ACME' };
      db.query.mockResolvedValueOnce({ rows: [mockCustomer] });

      const result = await GenericEntityService.findByField('customer', 'email', 'cust@example.com');
    ...
    test('should find role by name', async () => {
      ...
      const result = await GenericEntityService.findByField('role', 'name', 'admin');
    ...
    test('should return null when not found', async () => {
      ...
      const result = await GenericEntityService.findByField('user', 'email', 'nonexistent@example.com');
    ...
    test('should include LIMIT 1 in query', async () => {
      ...
      await GenericEntityService.findByField('user', 'email', 'test@example.com');
  ...
  describe('with RLS context', () => {
    test('should apply RLS filter when context provided', async () => {
      ...
      await GenericEntityService.findByField('user', 'email', 'test@example.com', rlsContext);
