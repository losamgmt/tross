...
describe('GenericEntityService.batch()', () => {
  ...
  describe('validation', () => {
    test('should throw error for empty operations array', async () => {
      await expect(GenericEntityService.batch('customer', []))
        .rejects.toThrow('Operations must be a non-empty array');
    });

    test('should throw error for non-array operations', async () => {
      await expect(GenericEntityService.batch('customer', 'not-array'))
        .rejects.toThrow('Operations must be a non-empty array');
    });

    test('should throw error for null operations', async () => {
      await expect(GenericEntityService.batch('customer', null))
        .rejects.toThrow('Operations must be a non-empty array');
    });
    ...
    test('should throw error for invalid entity name', async () => {
      await expect(GenericEntityService.batch('invalid_entity', [
  ...
  describe('update operations', () => {
    test('should update record and return new values', async () => {
      const oldRecord = { id: 1, email: 'old@test.com', company_name: 'Old Corp' };
      const newRecord = { id: 1, email: 'old@test.com', company_name: 'New Corp' };
  ...
  describe('mixed operations', () => {
    test('should handle create, update, delete in order', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.stats).toEqual({ created: 1, updated: 1, deleted: 1, failed: 0 });
  ...
  describe('result structure', () => {
    test('should return detailed results for each operation', async () => {
      ...
      expect(result).toMatchObject({
        success: true,
        results: [
          { index: 0, operation: 'create', success: true, result: expect.any(Object) },
          { index: 1, operation: 'create', success: true, result: expect.any(Object) },
        ],
        errors: [],
