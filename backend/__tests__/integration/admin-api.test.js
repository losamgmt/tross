...
describe("Admin API - Integration Tests", () => {
  const ctx = createTestContext({ roles: ["admin", "manager", "technician"] });

  beforeAll(() => ctx.setup());
  afterAll(() => ctx.teardown());

  afterEach(async () => {
    await ctx.pool.query("TRUNCATE TABLE audit_logs RESTART IDENTITY CASCADE");
  ...
  describe("Authorization", () => {
    test("should return 401 without authentication", async () => {
      const response = await ctx.get("/api/admin/system/sessions").unauthenticated().execute();
      expect(response.status).toBe(401);
    });

    test("should return 403 for non-admin users", async () => {
      const response = await ctx.get("/api/admin/system/sessions").as("technician").execute();
      expect(response.status).toBe(403);
    });

    test("should return 403 for manager users", async () => {
      const response = await ctx.get("/api/admin/system/sessions").as("manager").execute();
      expect(response.status).toBe(403);
    });

    test("should return 200 for admin users", async () => {
      const response = await ctx.get("/api/admin/system/sessions").as("admin").execute();
  ...
  describe("POST /api/admin/system/sessions/:userId/force-logout", () => {
    test("should suspend user and revoke their tokens", async () => {
      ...
      const userCheck = await ctx.pool.query("SELECT status FROM users WHERE id = $1", [testUser.user.id]);
      expect(userCheck.rows[0].status).toBe("suspended");

      const tokenCheck = await ctx.pool.query("SELECT is_active FROM refresh_tokens WHERE user_id = $1", [testUser.user.id]);
      expect(tokenCheck.rows[0].is_active).toBe(false);
    });

    test("should return 400 when trying to logout self", async () => {
      const response = await ctx
        .post(`/api/admin/system/sessions/${ctx.user("admin").id}/force-logout`)
        .as("admin")
        .execute();
  ...
  describe("POST /api/admin/system/sessions/:userId/reactivate", () => {
    test("should reactivate a suspended user", async () => {
      const testUser = await ctx.createUser("technician");
      await ctx.pool.query("UPDATE users SET status = $1 WHERE id = $2", ["suspended", testUser.user.id]);
  ...
  describe("DELETE /api/admin/system/sessions/:sessionId", () => {
    test("should revoke a specific session", async () => {
      ...
      const response = await ctx.delete(`/api/admin/system/sessions/${sessionId}`).as("admin").execute();

      expect(response.status).toBe(200);
      expect(response.body.success).toBe(true);

      const check = await ctx.pool.query("SELECT is_active FROM refresh_tokens WHERE id = $1", [sessionId]);
      expect(check.rows[0].is_active).toBe(false);
    });

    test("should return 404 for non-existent session", async () => {
      const response = await ctx.delete("/api/admin/system/sessions/99999").as("admin").execute();
      expect(response.status).toBe(404);
    });

    test("should return 400 for invalid session ID", async () => {
      const response = await ctx.delete("/api/admin/system/sessions/invalid").as("admin").execute();
      expect(response.status).toBe(400);
    });
  });
  ...
  describe("GET /api/admin/entities", () => {
    test("should return list of all entities", async () => {
      const response = await ctx.get("/api/admin/entities").as("admin").execute();
  ...
  describe("GET /api/admin/:entity", () => {
    test("should return entity metadata with RLS matrix", async () => {
      const response = await ctx.get("/api/admin/customer").as("admin").execute();
    ...
    test("should return 404 for unknown entity", async () => {
      const response = await ctx.get("/api/admin/nonexistent").as("admin").execute();
      expect(response.status).toBe(404);
    });

    test("should return raw metadata for /:entity/raw", async () => {
      const response = await ctx.get("/api/admin/customer/raw").as("admin").execute();
  ...
  describe("GET /api/admin/system/logs/data", () => {
    ...
    test("should return paginated data logs", async () => {
      const response = await ctx.get("/api/admin/system/logs/data").as("admin").execute();
    ...
    test("should filter by resourceType", async () => {
      const response = await ctx.get("/api/admin/system/logs/data?resourceType=customers").as("admin").execute();
    ...
    test("should filter by action", async () => {
      const response = await ctx.get("/api/admin/system/logs/data?action=create").as("admin").execute();
    ...
    test("should support pagination parameters", async () => {
      const response = await ctx.get("/api/admin/system/logs/data?page=1&limit=2").as("admin").execute();
  ...
  describe("GET /api/admin/system/logs/auth", () => {
    ...
    test("should return paginated auth logs", async () => {
      const response = await ctx.get("/api/admin/system/logs/auth").as("admin").execute();
    ...
    test("should filter by action type", async () => {
      const response = await ctx.get("/api/admin/system/logs/auth?action=login_failure").as("admin").execute();
  ...
  describe("GET /api/admin/system/logs/summary", () => {
    test("should return log summary for default period", async () => {
      const response = await ctx.get("/api/admin/system/logs/summary").as("admin").execute();
    ...
    test("should accept period parameter", async () => {
      const response = await ctx.get("/api/admin/system/logs/summary?period=week").as("admin").execute();
  ...
  describe("GET /api/admin/system/config/permissions", () => {
    test("should return permissions.json content", async () => {
      const response = await ctx.get("/api/admin/system/config/permissions").as("admin").execute();
  ...
  describe("GET /api/admin/system/config/validation", () => {
    test("should return derived validation rules from entity metadata", async () => {
      const response = await ctx.get("/api/admin/system/config/validation").as("admin").execute();
  ...
  describe("GET /api/admin/system/maintenance", () => {
    test("should return current maintenance mode status", async () => {
      const response = await ctx.get("/api/admin/system/maintenance").as("admin").execute();
  ...
  describe("PUT /api/admin/system/maintenance", () => {
    afterEach(async () => {
      await ctx.put("/api/admin/system/maintenance").as("admin").send({ enabled: false }).execute();
    });
    ...
    test("should disable maintenance mode", async () => {
      await ctx.put("/api/admin/system/maintenance").as("admin").send({ enabled: true }).execute();

      const response = await ctx.put("/api/admin/system/maintenance").as("admin").send({ enabled: false }).execute();
    ...
    test("should return 400 when enabled is not boolean", async () => {
      const response = await ctx.put("/api/admin/system/maintenance").as("admin").send({ enabled: "yes" }).execute();
      expect(response.status).toBe(400);
    });

    test("should return 400 when enabled is missing", async () => {
      const response = await ctx.put("/api/admin/system/maintenance").as("admin").send({ message: "No enabled field" }).execute();
      expect(response.status).toBe(400);
    });
  });
  ...
  describe("GET /api/admin/system/settings", () => {
    test("should return all system settings", async () => {
      const response = await ctx.get("/api/admin/system/settings").as("admin").execute();
  ...
  describe("GET /api/admin/system/settings/:key", () => {
    test("should return a specific setting", async () => {
      const response = await ctx.get("/api/admin/system/settings/maintenance_mode").as("admin").execute();
    ...
    test("should return 404 for unknown setting", async () => {
      const response = await ctx.get("/api/admin/system/settings/nonexistent_setting").as("admin").execute();
      expect(response.status).toBe(404);
    });
  });
  ...
  describe("PUT /api/admin/system/settings/:key", () => {
    test("should update a setting value", async () => {
      ...
      const response = await ctx.put("/api/admin/system/settings/feature_flags").as("admin").send({ value: newValue }).execute();
    ...
    test("should return 400 when value is missing", async () => {
      const response = await ctx.put("/api/admin/system/settings/feature_flags").as("admin").send({}).execute();
      expect(response.status).toBe(400);
    });
  });
});
