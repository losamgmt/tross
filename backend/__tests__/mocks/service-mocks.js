...
function createQueryBuilderMock(options = {}) {
  ...
  return {
     ...
     */
    buildFilterClause: jest.fn((filters = {}, filterableFields = [], paramOffset = 0) => {
      // ⚠️ Override escape hatch (for error simulation only!)
      if (overrides.buildFilterClause) {
        return overrides.buildFilterClause(filters, filterableFields, paramOffset);
      }
          ...
          Object.entries(value).forEach(([op, opValue]) => {
            ...
            switch (op) {
              ...
              case 'in':
                ...
                const placeholders = inValues.map((_, idx) => `$${currentOffset + idx}`).join(', ');
     ...
     */
    buildSortClause: jest.fn((sortBy, sortOrder, sortableFields = [], defaultSort = {}) => {
      // ⚠️ Override escape hatch (for error simulation only!)
      if (overrides.buildSortClause) {
        return overrides.buildSortClause(sortBy, sortOrder, sortableFields, defaultSort);
      }
      ...
      if (!isValidField && defaultSort.field) {
        ...
        order = (upperOrder === 'ASC' || upperOrder === 'DESC')
          ? upperOrder
...
function createPaginationMock(options = {}) {
  ...
  return {
    ...
    validateParams: jest.fn((params = {}) => {
      ...
      const page = Math.max(1, Math.min(parseInt(params.page, 10) || DEFAULTS.PAGE, Number.MAX_SAFE_INTEGER));
      const limit = Math.max(1, Math.min(parseInt(params.limit, 10) || DEFAULTS.LIMIT, maxLimit));
...
function createSmartMocks(options = {}) {
  return {
    queryBuilder: createQueryBuilderMock({ overrides: options.queryBuilderOverrides }),
    pagination: createPaginationMock({ overrides: options.paginationOverrides }),
  };
}
