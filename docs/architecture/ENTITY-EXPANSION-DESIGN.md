...
### 7. One Concept, One Expression (Revision April 7, 2026)

**Every semantic concept has exactly ONE canonical representation.**

| Concept | Canonical Form | Level | Replaces |
|---------|---------------|-------|----------|
| "Which field identifies this entity" | `identityField: 'name'` | Entity | `isIdentity` (never existed) |
| "Which fields to display in lists" | `displayFields: [...]` | Entity | Scattered display logic |
| "How this field's value is generated" | `autoGenerated: {...}` | Field | `identifierPrefix` (entity-level) |
| "What pattern names this entity" | `namePattern: 'human'` | Entity | Stays (describes entity, not field) |
| "This field cannot change" | `immutable: true` | Field | System constraint only |
| "Who can do what" | `access: {...}` | Field | Role-based access |
| "Block or require approval" | `beforeChange: [...]` | Field | `guards`, `approvalConfig` |
| "React after change" | `afterChange: [...]` | Field | `triggers`, `approvalConfig.onApprove` |
| "What kind of entity is this" | `entityTraits: {...}` | Entity | `isJunction`, `isSystemTable` |

### 8. Identity Configuration (Revision April 7, 2026)

**Key Insight:** Identity has two orthogonal concerns:
- **Entity-level:** WHAT identifies this entity (a pointer to which field)
- **Field-level:** HOW that field's value is computed (behavior)

These should NOT be conflated on the field.

#### Entity-Level Identity Properties

```javascript
module.exports = {
  // WHAT identifies this entity (pointer)
  identityField: 'invoice_number',     // Field used as unique identity
  
  // WHAT to show in UI (for display)
  displayFields: ['invoice_number'],   // Fields shown in lists/dropdowns
  // For HUMAN entities: ['first_name', 'last_name']
  
  // HOW the entity is named (pattern type, not on field)
  namePattern: NAME_PATTERNS.COMPUTED, // 'computed' | 'simple' | 'human' | null
  
  // ...
}
```

#### Field-Level Generation Properties

```javascript
fields: {
  // AUTO-GENERATED identity field
  invoice_number: withTraits(
    ...
    TRAITS.IMMUTABLE,
    TRAIT_SETS.IDENTITY,  // required, unique, searchable, filterable, sortable
  ),
  
  // USER-PROVIDED identity field (no autoGenerated)
  name: withTraits(
    { type: 'string', maxLength: 100 },
    TRAIT_SETS.IDENTITY,
  ),
  
  // HUMAN name pattern - multiple display fields
  first_name: withTraits(
    { type: 'string', maxLength: 50 },
    TRAITS.REQUIRED,
    TRAIT_SETS.DISPLAY,  // searchable, filterable, sortable
  ),
  ...
  email: withTraits(
    FIELD.EMAIL,
    TRAIT_SETS.IDENTITY,  // This is THE identity field
  ),
}
```
...
| Entity Type | namePattern | identityField | displayFields | autoGenerated |
|-------------|-------------|---------------|---------------|---------------|
| work_order | `computed` | `work_order_number` | `['work_order_number']` | `{ prefix: 'WO', template: '...' }` |
| customer | `human` | `email` | `['first_name', 'last_name']` | none |
| vendor | `simple` | `name` | `['name']` | none |
| audit_log | `null` | `id` | `['action', 'resource_type']` | none |
| customer_unit | `null` | none | `['customer_id', 'unit_id']` | none (junction) |

#### Validation Rules

- `identityField` MUST reference a field in `fields` (unless junction)
- `displayFields` MUST reference fields in `fields`
- If `namePattern: 'computed'`, identity field SHOULD have `autoGenerated`
- If `namePattern: 'human'`, `displayFields` should include name components
- Junctions may have `identityField: null` (identified by composite key)

### 9. Entity Traits (Added April 7, 2026)

**Key Principle:** Entity-level classification uses a composable trait system parallel to field traits.

#### The Problem with Current Approach

```javascript
// CURRENT: Ad-hoc booleans scattered across entities
isJunction: true,        // 5 entities
isSystemTable: true,     // 1 entity
uncountable: true,       // 2 entities
sharedPrimaryKey: true,  // 1 entity
```

These are inconsistent, incomplete, and not composable.

#### Two-Axis Classification

| Axis | Purpose | Type | Values |
|------|---------|------|--------|
| **Structure** | What the entity IS | Enum (exclusive) | `standard`, `junction`, `polymorphic` |
| **Traits** | How the entity BEHAVES | Set (additive) | `system`, `workflow`, `auditable`, etc. |

#### Entity Structure Types

```javascript
const ENTITY_STRUCTURE = Object.freeze({
  /** Normal business entity with identity, nav, full CRUD */
  STANDARD: 'standard',
  
  /** M:M pivot table, no nav, composite identity */
  JUNCTION: 'junction',
  
  /** Type-discriminated child (e.g., file_attachment) */
  POLYMORPHIC: 'polymorphic',
});
```

**Mutual Exclusion:** An entity has exactly ONE structure type.

#### Entity Behavioral Traits

```javascript
const ENTITY_TRAITS = Object.freeze({
  /** System table, not user-facing (audit_log, notification) */
  SYSTEM: Object.freeze({ system: true }),
  
  /** Has status-driven workflow (invoice, work_order) */
  WORKFLOW: Object.freeze({ workflow: true }),
  
  /** Changes are tracked in audit log */
  AUDITABLE: Object.freeze({ auditable: true }),
  
  /** Name has no plural form (inventory, preferences) */
  UNCOUNTABLE: Object.freeze({ uncountable: true }),
});
```

**Composition:** An entity can have ZERO or MANY behavioral traits.

#### Usage in Metadata

```javascript
const { ENTITY_STRUCTURE, ENTITY_TRAITS, withEntityTraits } = require('../entity-traits');

module.exports = {
  entityKey: 'invoice',
  
  // Structural classification (REQUIRED, exactly one)
  structure: ENTITY_STRUCTURE.STANDARD,
  
  // Behavioral traits (OPTIONAL, composable)
  traits: withEntityTraits(
    ENTITY_TRAITS.WORKFLOW,
    ENTITY_TRAITS.AUDITABLE,
  ),
  // Result: { workflow: true, auditable: true }
  
  // ...
};
```

#### Migration from Current Properties

| Current Property | New Location | Notes |
|-----------------|--------------|-------|
| `isJunction: true` | `structure: ENTITY_STRUCTURE.JUNCTION` | Structure type |
| `junctionFor: {...}` | `junction: { entities, uniqueOn }` | Junction config (unchanged) |
| `isSystemTable: true` | `traits: { system: true }` | Behavioral trait |
| `uncountable: true` | `traits: { uncountable: true }` | Behavioral trait |
| `sharedPrimaryKey: true` | `structure: ENTITY_STRUCTURE.STANDARD` + note | Rare, keep as-is initially |

#### Deriving Behavior from Traits

```javascript
// In consumers
function isWorkflowEntity(metadata) {
  return metadata.traits?.workflow === true;
}

function isUserFacing(metadata) {
  return metadata.traits?.system !== true;
}

function requiresNavigation(metadata) {
  return metadata.structure === ENTITY_STRUCTURE.STANDARD && 
         metadata.traits?.system !== true;
}
```

#### Validation Rules

- Every entity MUST have `structure` (default: `ENTITY_STRUCTURE.STANDARD`)
- `traits` defaults to `{}` (no traits)
- If `structure: JUNCTION`, must have `junction: { entities, uniqueOn }`
- If `structure: JUNCTION`, must NOT have `navigation`
- If `traits.system: true`, must have `navVisibility: null`
...
#### 2.1 Identity Configuration (REVISED April 7, 2026)
**Entity-level (pointers — KEEP):**
- [ ] Standardize `identityField` presence on all non-junction entities
- [ ] Standardize `displayFields` as array of field names for UI display
- [ ] Keep `namePattern` enum (COMPUTED, SIMPLE, HUMAN, null) at entity level

**Field-level (behavior — NEW):**
- [ ] Add `autoGenerated: true` trait to fields whose values are computed (prefixed IDs, timestamps)
- [ ] Validation: `identityField` must reference an existing field
- [ ] Validation: `displayFields` entries must reference existing fields
...
#### 2.6 Entity Traits System (NEW — April 7, 2026)

**Structure Types (mutually exclusive):**
- [ ] Add `structureType` property to all entities (STANDARD | JUNCTION | POLYMORPHIC)
- [ ] Default: STANDARD when omitted
- [ ] Validation: exactly one structure type per entity
- [ ] Remove `isJunction` boolean (derive from structureType === 'JUNCTION')

**Behavioral Traits (composable):**
- [ ] Add `traits` array property to entities (optional)
- [ ] Allowed values: 'system', 'workflow', 'auditable', 'uncountable'
- [ ] Validation: traits array contains only allowed values
- [ ] Remove `isSystemTable`, `uncountable` booleans (derive from traits)

**Migration steps:**
- [ ] Add ENTITY_STRUCTURE enum to constants
- [ ] Add ENTITY_TRAITS enum to constants
- [ ] Update entity-metadata-validator.js with new properties
- [ ] Add helper functions: `hasStructure(entity, type)`, `hasTrait(entity, trait)`
- [ ] Migrate all 34 entities to explicit structureType
- [ ] Migrate applicable entities to traits array
...
fieldName: {
  ...
  autoGenerated: false,    // True for computed values (prefixed IDs, timestamps)
...
**Identity stays at entity level (pointers, not field behavior):**
```javascript
{
  identityField: 'work_order_number',  // Which field identifies the entity
  displayFields: ['name'],              // Which fields shown in UI
  namePattern: 'COMPUTED',              // How identity field is named (COMPUTED|SIMPLE|HUMAN|null)
}
```
...
### Entity Traits (NEW April 7, 2026)
```javascript
// Structure Types (mutually exclusive — what KIND of entity)
ENTITY_STRUCTURE = { STANDARD, JUNCTION, POLYMORPHIC }

// Behavioral Traits (composable — what BEHAVIORS apply)
ENTITY_TRAITS = { SYSTEM, WORKFLOW, AUDITABLE, UNCOUNTABLE }

// Usage in metadata
{
  structureType: 'JUNCTION',           // Default: STANDARD
  traits: ['system', 'uncountable'],   // Optional behavioral traits
}

// Helper functions
hasStructure(entity, 'JUNCTION')       // true if structureType === 'JUNCTION'
hasTrait(entity, 'workflow')           // true if traits includes 'workflow'
```
...
## Frontend Test SSOT Migration (Added April 7, 2026)

### Problem Statement

Frontend tests use a static `allKnownEntities` list for parameterized testing:

```dart
// frontend/test/factory/entity_registry.dart
const allKnownEntities = <String>[
  'approval_request', 'asset', 'audit_log', /* ... 34 total */
];
```

This violates SSOT — adding an entity to backend metadata doesn't automatically include it in tests.

### Solution: Dynamic Entity List from SSOT

Replace static list with dynamic access to `EntityTestRegistry.allEntityNames`:

```dart
// BEFORE: Static list at declaration time (SSOT violation)
for (final entity in allKnownEntities) {
  test('$entity has valid fields', () { ... });
}

// AFTER: Dynamic list from metadata (SSOT compliant)
...
1. **One test per entity** — Each entity appears as a separate test case in CI output
2. **Dynamic from JSON** — Entity list read from `entity_metadata.json` at runtime
3. **Category visibility** — Test names include entity category for filtering
4. **Soft assertions** — Collect ALL failures before reporting
...
void main() {
  ...
  // Generate one test per entity dynamically
  for (final entityName in EntityTestRegistry.allEntityNames) {
    final meta = EntityTestRegistry.getMetadata(entityName);
    final category = _deriveCategory(meta);
    
    test('[$category] $entityName - field parity', () {
      final failures = <String>[];
      
      // Check 1: All fields have type
      for (final field in meta.fields.entries) {
        if (field.value.type == null) {
          failures.add('${field.key}: missing type');
        }
      }
      
      // Check 2: Required fields are not empty
      // ... more checks
...
String _deriveCategory(EntityMetadata meta) {
  if (meta.isJunction) return 'junction';
  if (meta.navVisibility == null) return 'system';
  // Derive workflow from status field having non-active values
  final status = meta.fields['status'];
  if (status != null && 
      status.enumValues != null && 
      !status.enumValues!.contains('active')) {
    return 'workflow';
  }
  return meta.namePattern ?? 'standard';
}
```
...
```
✓ [computed] approval_request - field parity
✓ [simple] asset - field parity
✓ [system] audit_log - field parity
✓ [workflow] invoice - field parity
✗ [junction] customer_unit - field parity
    Reason: customer_id: missing displayField
            unit_id: missing displayField
```

### Filtering by Category

```bash
# Run only workflow entity tests
flutter test --name "workflow"

# Run only junction tests
flutter test --name "junction"
```
...
| Phase | Scope | Files |
|-------|-------|-------|
| 1 | Remove `allKnownEntities` constant | `entity_registry.dart` |
| 2 | Update 6 files using the constant | See list below |
| 3 | Add `_deriveCategory` helper | `entity_registry.dart` |
| 4 | Add soft assertion pattern | Per test file |

**Files to migrate:**
- `test/scenarios/filterable_data_table_test.dart` (3 usages)
- `test/scenarios/generic_entity_service_test.dart` (2 usages)
- `test/scenarios/service_contract_test.dart` (1 usage)
- `test/scenarios/validation_test.dart` (6 usages)
- `test/scenarios/widget_entity_test.dart` (2 usages)
- `test/scenarios/widget_state_permutation_test.dart` (1 usage)
...
/// @deprecated Will be removed after all usages migrated
const allKnownEntities = <String>[...];
```

### Validation Hook

Add CI check that `allKnownEntities` matches JSON until fully migrated:

```dart
test('allKnownEntities matches entity_metadata.json', () {
  final fromJson = EntityTestRegistry.allEntityNames.toList()..sort();
  final fromConst = allKnownEntities.toList()..sort();
  expect(fromConst, equals(fromJson), 
    reason: 'allKnownEntities is out of sync with metadata JSON');
});
```
