...
const { buildCompositeSchema } = require('../backend/utils/validation-loader.js');
...
console.log('   ' + (validRole.error ? '✗ FAIL: ' + validRole.error.message : '✓ PASS'));

// Test 2: Role with invalid status
const invalidRole = roleSchema.validate({ name: 'Admin', status: 'invalid_status' });
console.log('2. createRole with status="invalid_status":');
console.log('   ' + (invalidRole.error ? '✓ PASS (correctly rejected): ' + invalidRole.error.details[0].message : '✗ FAIL (should reject)'));
...
console.log('   ' + (validWO.error ? '✗ FAIL: ' + validWO.error.message : '✓ PASS'));
...
console.log('   ' + (wrongWO.error ? '✓ PASS (correctly rejected)' : '✗ FAIL (should reject - active is not valid WO status)'));

// Test 5: Technician with valid status
const techSchema = buildCompositeSchema('createTechnician');
const validTech = techSchema.validate({ first_name: 'John', last_name: 'Doe', email: 'john@test.com', status: 'available' });
console.log('5. createTechnician with status="available":');
console.log('   ' + (validTech.error ? '✗ FAIL: ' + validTech.error.message : '✓ PASS'));
