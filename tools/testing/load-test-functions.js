...
async function generateDispatcherToken(context, events, done) {
  if (cachedTokens.dispatcher && Date.now() - cachedTokens.dispatcher.timestamp < 60000) {
    context.vars.dispatcherToken = cachedTokens.dispatcher.token;
    return done();
  }

  try {
    const response = await axios.get(`${BACKEND_URL}/api/dev/token?role=dispatcher`);
    const token = response.data.token;
...
async function generateCustomerToken(context, events, done) {
  if (cachedTokens.customer && Date.now() - cachedTokens.customer.timestamp < 60000) {
    context.vars.customerToken = cachedTokens.customer.token;
    return done();
  }

  try {
    const response = await axios.get(`${BACKEND_URL}/api/dev/token?role=customer`);
    const token = response.data.token;
...
async function getTestCustomerId(context, events, done) {
  try {
    ...
    const response = await axios.get(`${BACKEND_URL}/api/customers?page=1&limit=1`, {
...
async function getTestWorkOrderId(context, events, done) {
  try {
    ...
    const response = await axios.get(`${BACKEND_URL}/api/work_orders?page=1&limit=1`, {
      headers: { Authorization: `Bearer ${token}` }
    });
